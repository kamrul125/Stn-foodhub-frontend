...
import { Menu, X, User, LogOut, LayoutDashboard, ShoppingBag, Info, PhoneCall, Utensils } from "lucide-react";
...
const Navbar = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(true); // পরে Auth logic দিয়ে রিপ্লেস হবে

  // আপনার ফোল্ডার অনুযায়ী নিখুঁত পাথ
  const navLinks = [
    { name: "Home", href: "/" },
        ...
        <nav className="hidden md:flex items-center space-x-8 text-sm font-bold">
          {navLinks.map((link) => (
            <Link 
              key={link.href} 
              href={link.href} 
              className="transition-colors hover:text-orange-600 text-foreground/70 relative after:absolute after:bottom-1after:left-0 after:h-2px after:w-0 hover:after:w-full after:bg-orange-600 after:transition-all"
            >
              {link.name}
            </Link>
          ))}
        ...
        <div className="flex items-center space-x-3">
          ...
          {isLoggedIn ? (
            <div className="flex items-center space-x-3">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="icon" className="rounded-full border-2 border-orange-600/20 hover:border-orange-600 transition-all overflow-hidden">
                    <User className="h-5 w-5 text-orange-600" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-64 p-2 rounded-1.5rem shadow-2xl border-slate-100">
                  <DropdownMenuLabel className="font-black italic px-4 py-3">Account Overview</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  
                  <DropdownMenuItem asChild className="rounded-xl focus:bg-orange-50 focus:text-orange-600 py-3 cursor-pointer">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem 
                    className="text-red-600 font-bold rounded-xl focus:bg-red-50 focus:text-red-600 py-3 cursor-pointer" 
            ...
            <div className="hidden md:flex items-center space-x-3">
              ...
              <Link href="/register">
                <Button className="bg-orange-600 hover:bg-orange-700 rounded-xl font-bold text-white shadow-lg shadow-orange-100 transition-all">
                  Get Started
                </Button>
              </Link>
            </div>
          ...
          <nav className="flex flex-col space-y-4">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-xl font-black italic border-b border-slate-100 pb-3 flex justify-between items-center group"
                onClick={() => setIsOpen(false)}
              >
                {link.name}
                <div className="h-2 w-2 rounded-full bg-orange-600 opacity-0 group-hover:opacity-100 transition-all" />
              </Link>
            ))}
          </nav>
          ...
          <div className="pt-4">
            {!isLoggedIn ? (
              ...
              <Link href="/dashboard/admin" onClick={() => setIsOpen(false)}>
                <Button className="w-full h-14 bg-slate-900 rounded-2xl font-black italic text-white flex items-center justify-center gap-2">
